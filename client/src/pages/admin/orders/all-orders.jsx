...
const AllOrders = () => {
  ...
  const placeOrder = data.filter(data => data.status === 'order-placed');
  ...
  return (
    <div>
      ...
      <h2 className="font-bold text-2xl mb-5">All Order placed Details</h2>
      ...
      <div className="flex flex-col gap-3 mt-3">
        ...
        {placeOrder?.length === 0 ? (
          <p>No order placed data today!</p>
        ) : (
          placeOrder?.map((items) => {
            return (
              <div className=" bg-white p-4 border shadow-sm" key={items?._id}>
                ...
                {items?.status === "delivered" ? null : (
                  <button className="bg-sky-800 py-2 border shadow-sm print:hidden rounded-md text-white font-bold w-full mt-5">
